...
c	modified for hp......output near line 6000 (error) directed to 
c	logical unit 6
...
C----------COMMON AREAS.
      ...
      COMMON /mcrun/  n_mcrun,yes_monte             ! for monte carlo
...
C----------COMMON AREAS.
      ...
      COMMON /bessel2/ bl1,bl2,bl3,bl4,bl5,           !Eval of function bl(z).
...
C----------COMMON AREAS.
      ...
      COMMON /mcrun/  n_mcrun,yes_monte
C==========================DECLARATION DIVISION=================================
...
c      WRITE(3,122) n_mcrun
c 122  format(/'MC run number from nucCOMmcb.f = ',i4/)

      CALL check1   ! THIS WILL ACCESS THE RATE VARIATIONS IN THE 
                    ! MONTE CARLO ROUTINE. AT THIS POINT IT DOES NOT
                    ! KNOW THE MC RUN NUMBER 
      CALL sol(loop)
...
cccc  REAL FUNCTION eval
ccccccccccccccccccccccc omit entry statements ccccccccccccccccccc
ccccccccccccc make 6 new functions here func1(x),...,func6(x) cccc
...
c      REAL   func1                 !1st part n->p rate.
c      REAL   func2                 !2nd part n->p rate.
c      REAL   func3                 !1st part p->n rate.
c      REAL   func4                 !2nd part p->n rate.
c      REAL   func5                 !Energy density for neutrinos.
c      REAL   func6                 !Energy density for antineutrinos.
...
ccccccccccccccccc      ENTRY func3(x)
      IF (x.le.1.) THEN
        func3 = 0.
      ELSE
        part1 = 1./(1.+ex(-.511*x/t9mev))
...
ccccccccccccccccccccccccc      ENTRY func4(x)
      IF (x.le.1.) THEN
        func4 = 0.
      ELSE
        part1 = 1./(1.+ex(+.511*x/t9mev))
...
ccccccccccccccccccc      ENTRY func5(x)
...
ccccccccccccccccccc      ENTRY func6(x)
...
C-----------COMMON AREAS.
      ...
      COMMON /therm2/  thm(14),hubcst                 !Dynamic variables.
      COMMON /endens/ rhone0,rhob0,rhob,rnb          !Energy densities.
...
C60--------POSSIBLE ERROR MESSAGES AND EXIT-------------------------------------
      ...
      IF (mbad.ne.0) THEN          !Problem in gaussian elimination.
        IF (mbad.eq.-1) WRITE (6,6000) ierror !Error message.
        IF (mbad.ge. 1) WRITE (6,6002) mbad   !Error message.
